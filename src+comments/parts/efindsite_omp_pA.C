...
int main(int argc, char *argv[])
{
    ...
    time_t t_beg, t_end, t_bench1, t_bench2;
    ...
    for ( int i = 0; i < argc; i++ )
    {
        ...
        { target_name    = string(argv[i+1]); target_opt    = true; }
        if ( !strcmp(argv[i],"-i") && i < argc ) 
        { psipred_name   = string(argv[i+1]); psipred_opt   = true; }
        if ( !strcmp(argv[i],"-e") && i < argc ) 
        { sequence_name  = string(argv[i+1]); sequence_opt  = true; }
        if ( !strcmp(argv[i],"-t") && i < argc ) 
        { templates_name = string(argv[i+1]); templates_opt = true; }
        if ( !strcmp(argv[i],"-o") && i < argc ) 
        { output_name    = string(argv[i+1]); output_opt    = true; }
        if ( !strcmp(argv[i],"-l") && i < argc ) 
        { cmps_name      = string(argv[i+1]); cmps_opt      = true; }
        if ( !strcmp(argv[i],"-b") && i < argc ) 
        { cut_seqid      = atof(argv[i+1]);                         }
        if ( !strcmp(argv[i],"-m") && i < argc ) 
        { cut_tmscore    = atof(argv[i+1]);                         }
        if ( !strcmp(argv[i],"-x") && i < argc ) 
        { cut_templates  = atoi(argv[i+1]);                         }
        if ( !strcmp(argv[i],"-r") && i < argc ) 
        { cut_binrest    = atof(argv[i+1]);                         }
        if ( !strcmp(argv[i],"-n") && i < argc ) 
        { cut_binresn    = atoi(argv[i+1]);                         }
        if ( !strcmp(argv[i],"-f") && i < argc ) 
        { cut_clustlig   = atof(argv[i+1]);                         }
        if ( !strcmp(argv[i],"-c") && i < argc ) 
        { met_clustlig   = string(argv[i+1]);                       }
        if ( !strcmp(argv[i],"-d") && i < argc ) 
        { cut_clustdis   = atof(argv[i+1]);                         }

#ifdef __linux

        if ( !strcmp(argv[i],"-g") && i < argc ) 
        { met_clustdis   = string(argv[i+1]);                       }
    ...
    path1 = getenv("EF_LIB"); if ( path1==NULL ) 
    { cout << "EF_LIB is not set" << endl; exit(EXIT_FAILURE); }

    path1 = getenv("EF_MAP"); if ( path1==NULL ) 
    { cout << "EF_MAP is not set" << endl; exit(EXIT_FAILURE); }

    path1 = getenv("EF_MOD"); if ( path1==NULL ) 
    { cout << "EF_MOD is not set" << endl; exit(EXIT_FAILURE); }

#ifdef __linux

    path1 = getenv("EF_APL"); if ( path1==NULL ) 
    { cout << "EF_APL is not set" << endl; exit(EXIT_FAILURE); }
...
#endif
// these lines can probably be moved to part3
    ...
    if ( cut_seqid < 1.0 )
        cout << "!!! Benchmarking mode activated with max sid of " 
             << cut_seqid << " !!!" << endl << endl;

    if ( cut_tmscore < 0.4 )
        cout << "!!! TMscore of " << cut_tmscore 
             << " is below the statistical significance threshold !!!" << endl << endl;
    ...
    {
        cout << "!!! Threshold for binding residues must be <=1, setting to 0.18 !!!" 
             << endl << endl;
    ...
    {
        cout << "!!! Fingerprint clustering method must be either T or A, setting to T !!!" 
             << endl << endl;
    ...
    {
        cout << "!!! Pocket clustering method must be either P or L, setting to P !!!" 
             << endl << endl;
    ...
    {
        cout << "!!! Max number of templates exceeded, setting to " 
             << MAXTPL << " !!!" << endl << endl;
    ...
    /*
     * this function will search through templates and assign best probabilities for
     * each protein template family
     * prob1 is probability of ligand binding
     * prob2 is probability of tanimoto score >= 0.5
     */
    ...
    /*
     * every template in template_list must be scored using alignNW
     * alignNW basically gives a score based on best alignment
     * if this score is less then set threshold (i.e. not too long)
     * it will be added to template_set
     */
    for ( it1 = template_list.begin(); it1 != template_list.end(); it1++ )
        if ( template_set.size() < cut_templates )
        {
            Template * template_tmp = new Template( 0, 0, 0, 
                        template_prob1.find(*it1)->second, template_prob2.find(*it1)->second );

            /* load fingerprint data for the template i
             */
            time(&t_beg);
            bool load1 = template_tmp->loadTemplate( lib_path+"/data/"+(*it1).substr(1,2)
                                                        +"/"+(*it1), template_list );
            time(&t_end);
            template_load_sum += difftime(t_end, t_beg);

            if ( !load1 ) // no error in loading template
            {
                /* score template */
                double sid1 = template_tmp->alignNW( target->getProteinSequence() );

                /* insert to template_set if below threshold */
                if ( sid1 <= cut_seqid )
                {
                    fprintf(to_disk, "%s\n", (*it1).c_str());
                    //printf("%s\n", (*it1).c_str());
                    template_set.insert( std::pair< int,Template * >
                    ( template_tmp->getProteinResiduesTotal(), template_tmp ) );
                }
            }
        }
    ...
    /* store offsets for each template based on number of residues in each template
     */
    ...
    time(&t_bench2); // end filtering and give output

    cout << ". " << template_set.size() << " templates survived (" 
         << fixed << setprecision(0) << difftime(t_bench2, t_bench1) << " s)" << endl << endl;

    printf("\t*********\t total load time: %.0f\t*********\n\n", template_load_sum);

    cout << "Estimating data size ... " << flush;

    time(&t_bench1); // begin estimation of data size, also prep for frtmalign
    ...
    int i_offset = 0; // offset for ith template set protein
    ...
    for ( tpl1 = template_set.begin(); tpl1 != template_set.end(); tpl1++ )
    {
        ...
        /* copy protein sequence of ith(i.e. i_offset) template set protein
         */
        for ( int t_i = 0; t_i < ((*tpl1).second)->getProteinResiduesTotal(); t_i++ )
            t_seq2[t_i+t_offset[i_offset]] = t_seqt[t_i];

        /* record the residue count for ith template protein
         */
        for ( int t_i = 0; t_i < ((*tpl1).second)->getProteinResiduesTotal(); t_i++ )
            t_res2[t_i+t_offset[i_offset]] = t_i + 1;

        /*
         * temporary array for triplets of doubles for each residue of the protein
         */
        double *t_xyzt = new double [((*tpl1).second)->getProteinResiduesTotal() * 3];

        ((*tpl1).second)->getProteinCoords1D(t_xyzt);

        /* store all the protein coords for each residue of template protein(ligand)
         */
    ...
    int mem_sco = n_set * sizeof(t_sco1) + n_set * sizeof(t_sco2) 
                + n_set * sizeof(t_sco3) + n_set * sizeof(t_sco4);
    int mem_ali = n_tar * n_set * sizeof(t_alig);
    int mem_mat = n_set * 12 * sizeof(t_rmat);

    time(&t_bench2); // end estimation of data size

    cout << setprecision(1) 
         << ( (double) ( mem_len + mem_seq + mem_res + mem_xyz 
                       + mem_sco + mem_ali + mem_mat ) ) / 1e6 
         << "M (" << fixed << setprecision(0) << difftime(t_bench2, t_bench1) << " s)" 
         << endl << endl;

    /*
     * Write following files to disk (simple text format)
     *      t_len1 int: length of target unknown protein
     *      t_seq1 char[t_len]: seq of residues(amino acids) for target unknown protein
     *      t_res1 int[n_tar]: the val = idx+1
     *      t_xyz1 double[n_tar*3]: coords for each amino acid of target unknown protein
     *
     *      n_set int: number of items in the template_set
     *      n_offset int: total number of 
     *      s_offset int: total number of residues across all template_set proteins
     *      t_offset int[]: holds offset value for beginning of each template protein
     *
     *      t_len2 int[]: array, length of each template protein sequence
     *      t_seq2 char[s_offset]: seq for all the template proteins 
     *      t_res2 int[]: same as t_res1 but for all template proteins
     *      t_xyz2 double[n_off*3]: coords for amino acids of all template proteins
     */
