...
int main(int argc, char *argv[])
{
    ...
    /*****
     * for daniel's function call benchmarks
     */
    time_t t_beg, t_fin;
    ...
    to_disk = fopen("part1.template.set.out", "w");
    /*
    ...
    for ( it1 = template_list.begin(); it1 != template_list.end(); it1++ )
        if ( template_set.size() < cut_templates )
        {
            ...
            time(&t_fin);
            template_load_sum += difftime(t_fin, t_beg);
    ...
    cout << setprecision(1) << ( (double) ( mem_len + mem_seq + mem_res + mem_xyz +
    mem_sco + mem_ali + mem_mat ) ) / 1e6 << "M (" << fixed << setprecision(0) <<
    difftime(t_bench2, t_bench1) << " s)" << endl << endl;

    cout << "Calculating alignments ... " << flush;

    time(&t_bench1); // start alignment calculations
     ...
     *      t_seq1 c_str: seq of residues(amino acids) for target unknown protein
     ...
     *      t_seq2 double[]: seq for all the template proteins 
...
#pragma omp parallel for schedule(dynamic) private(tm_i)
     ...
     *      t_sco4
     */
    to_disk = fopen("part2_from_original.out", "w");

    // save t_len1 
    fprintf(to_disk, "%d ", t_len1);
    ...
    /* read the following vars from disk
     *      t_alig
     *      t_rmat
     *      t_sco1
     *      t_sco2
